...
type AdminStatus = 'New' | 'Processing' | 'Done';
...
export default function AdminDashboard() {
  ...
  const handleBatchAction = async (batch: AdminBatch, action: Exclude<AdminAction, 'Reject'>) => {
    const label = batch.records.length > 1 ? `批次 ${batch.batchId || batch.key}` : `物料 ${batch.partNumbers[0]}`;
    if (!confirm(`确认要将 ${label} 的 ${batch.records.length} 条记录标记为 ${action} 吗？`)) return;
    ...
    try {
      const res = await fetch('/api/admin/mds', {
        ...
        body: JSON.stringify({
          ...
          recordId: batch.batchId ? undefined : batch.records[0].id,
          action,
          comment: '',
        }),
      });
  ...
  const handleRecordAction = async (record: AdminRecord, action: Exclude<AdminAction, 'Reject'>) => {
    if (!confirm(`确认要将物料 ${record.partNumber} / PARMA ${record.supplierCode} 标记为 ${action} 吗？`)) return;
    ...
    try {
      ...
      if (!res.ok) throw new Error('Admin record action failed');
      alert('操作成功！');
      fetchRecords();
    } catch {
      alert('操作失败');
          ...
          {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="text-center py-16 text-sm text-gray-400">暂无匹配批次</td>
                  </tr>
                ) : filtered.map(batch => (
                    ...
                    <tr className="border-b border-gray-50 last:border-none hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-4 px-6">
                        <div className="flex flex-col items-start gap-1.5">
                          ...
                          {batch.adminComment && batch.status === 'New' && (
